...
var assetLoader = (function() {
  ...
  this.imgs        = {
    ...
    'grass'         : 'imgs/grassMid1.png',
    'avatar_normal' : 'imgs/normalwalk.png',
   ...
   //sounds dictionary
  this.sounds      = {
    'bg'            : 'sounds/track2.mp3',
    'jump'          : 'sounds/pulo.mp3',
    'gameOver'      : 'sounds/fim.mp3'
  };
...
var player = (function(player) {
  ...
  // spritesheets
  player.sheet     = new SpriteSheet('imgs/normalwalk.png', player.width, player.height);
...
function getType() {
  ...
  if (platformLength === 2 && platformHeight < 3 && rand(0, 3) === 0) {
   type = 'cliff';
  }
...
function updateGround() {
  ...
  for (var i = 0; i < ground.length; i++) {
    ...
    if (player.minDist(ground[i]) <= player.height/2 + platformWidth/2 &&
      ...
      player.isJumping = false;
      player.isFalling = false;
      player.y = ground[i].y - player.height + 2;
      player.dy = 0;
    }
  }
...
/**
 * Funcao que cria inimigos*/

function spawnEnemySprites() {
  if (score < -150 && Math.random() > 0.96 && enemies.length < 3 && platformLength > 5 &&
...
function animate() {
  if (!stop) {
    ...
    // aumenta a velocidade quando o jogador está pulando
    if (ticker > (Math.floor(platformWidth / player.speed) * player.speed * 20) && player.dy !== 0) {
      ...
      // cria uma plataforma para encher o buraco criado pelo aumento da velocidade
      if (gapLength === 0) {
        var type = getType();
        ground.push(new Sprite(
          canvas.width + platformWidth % player.speed,
...
var myVar = setInterval(spostaDiv, 70);
var margin = 0;
...
/**
 * Espera o carregamento dos assets e mostra menu
 */
function mainMenu() {
...
/**
 * Comeca o jogo, reseta todas entidades/variaveis e cria chao e obstaculos
 */
function startGame() {
